...
脚本会重新解析 CSV、复制最新卡图、覆盖生成 `js/data.js`。

### 关于卡图映射

50 张卡图原文件名是 `模板.png … 模板52.png`（缺 21、22），顺序为「红→黄→蓝→绿→紫，每色 5 领袖，每领袖 主牌+绑定副牌」。这个映射写死在 `build.py` 的 `IMAGE_MAP` 里。**如果将来卡图导出的顺序或数量变了，需要相应修改 `IMAGE_MAP`。**

### 关于无卡图的 4 张

唐纳德鸭、永远在赢/反移民政策、肥头、肥头二号（均为 SP 卡）暂无卡图，站内显示「暂无卡图」占位。等有图后放进 `4.25导出_圆角`（或直接放进 `assets/cards/` 命名为对应编号 `.png`），并在 `build.py` 的 `IMAGE_MAP` 补上映射即可。
