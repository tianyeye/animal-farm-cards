...
某些卡有异画（备用画面），网页详情里会出现「⇆ 切换异画」按钮、图鉴缩略图带「异画」角标。做法：

1. 把异画原图（**带 3mm 出血**的成品，如 815×1110）放进 `assets/`。
2. 在 `tools/optimize-images.mjs` 的 `ALT_MAP` 里加一条「卡编号 → 源文件名」。
3. 跑 `node tools/optimize-images.mjs` → 会自动裁掉出血、圆角、加水印，输出 `assets/cards/<编号>_alt.webp`；再 `python build.py`。

> 异画原图是未加水印的源，已在 `.gitignore` 里排除（不提交、不公开），只发布处理后的 `_alt.webp`。
